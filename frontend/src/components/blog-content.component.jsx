...
const BlogContent = ({ block }) => {
    let { type, data } = block;
    //nếu block là văn bản thì nhúng html bên trong data.text
    if (type === "paragraph") {
        return <p dangerouslySetInnerHTML={{ __html: data.text }}></p> //dangerouslySetInnerHTML dùng khi bạn có HTML dạng string và muốn nó hiển thị đúng format (bold, link, xuống dòng…)
    }
    if (type === "header") {
        if (data.level === 3) {
            return <h3 className="text-3xl font-bold" dangerouslySetInnerHTML={{ __html: data.text }}></h3>
        }
        return <h1 className="text-4xl font-bold" dangerouslySetInnerHTML={{ __html: data.text }}></h1>
    }
    if (type === "image") {
