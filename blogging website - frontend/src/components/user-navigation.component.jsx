    ...
    return (
        ...
        <AnimationWrapper className="absolute right-0 z-50" transition={{ duration: 0.2 }}>
            ...
            >
                ...
                {/* Link Profile */}
                <Link className="text-white link pl-8 py-4" to={`/user/${username}`}>
                    Profile
                </Link>

                {/* Link Dashboard */}
                <Link className="text-white link pl-8 py-4" to={"/user/dashborad"}>
                    DashBoard
                </Link>

                {/* Link Setting */}
                <Link className="text-white link pl-8 py-4" to={"/user/setting"}>
                    Setting
                </Link>

                {/* Dòng phân cách */}
                <span className="text-white block border-t border-grey w-full"></span>

                {/* Nút Sign Out */}
                <button
                    onClick={signOutUser}
                    className="text-white text-left w-full pl-8 py-4 hover:bg-grey duration-200"
                >
                    <h1 className="font-bold text-xl text-white">Sign Out</h1>
